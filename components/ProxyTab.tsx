...
const ProxyTab: React.FC<ProxyTabProps> = ({ customApps, onAddApp, activeUrl, onNavigate }) => {
  ...
  // Sync internal state with browser fullscreen
  useEffect(() => {
  ...
  const processUrl = (target: string) => {
    ...
    // If ultraviolet is active (simulated here), prefix the URL
    // Standard Ultraviolet integration usually looks like this:
  ...
  const handleQuickCreate = () => {
    if (url) {
      setNewUrl(url);
      setNewName(url.split('.')[0].replace('https://', '').replace('http://', '').toUpperCase());
    ...
    return (
      ...
      >
        {!isFullscreen && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 pointer-events-none">
            <div className="bg-black/85 backdrop-blur-2xl border border-accent/40 rounded-full px-8 py-2 text-[11px] font-black text-accent uppercase tracking-widest shadow-glow flex items-center gap-3">
              ...
              Ultraviolet Engine: {new URL(activeUrl.startsWith('/') ? window.location.origin + activeUrl : activeUrl).hostname}
            </div>
          </div>
        )}
        ...
        <button 
          ...
          className="absolute top-6 right-6 bg-black/60 backdrop-blur-md border border-accent/40 text-accent w-12 h-12 rounded-xl flex items-center justify-center hover:bg-accent hover:text-white transition-all z-50 group shadow-2xl"
        >
        ...
        <button 
          ...
          className="absolute top-6 right-20 bg-black/60 backdrop-blur-md border border-accent/40 text-accent w-12 h-12 rounded-xl flex items-center justify-center hover:bg-accent hover:text-white transition-all z-50 shadow-2xl"
        >
          {isFullscreen ? (
    ...
    <div className="flex flex-col items-center space-y-16 py-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="flex flex-col items-center">
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.6em] mb-4">Neural Gateway System v.4.0.0</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/40 animate-in fade-in duration-300">
          <div className="glass max-w-md w-full p-8 rounded-[2.5rem] border border-accent/40 shadow-[0_0_100px_rgba(0,0,0,0.8)] space-y-8">
            <div className="space-y-2">
              <h3 className="text-3xl font-black italic uppercase text-accent tracking-tighter">ADD_CUSTOM_NODE</h3>
